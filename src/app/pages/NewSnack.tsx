...
import { StorageSnacks } from "@/src/storage/snackStorage";
...
export default function NewSnack() {
  const {
    ...
    handleSubmit,
    formState: { errors }
  } = useForm<FormEditProps>({ resolver: yupResolver(editSchema) });
  const { showErrorToast } = useToastNotification();
  const [isLoading, setIsLoading] = useState(false);
  const [selectedInput, setSelectedInput] = useState<"in" | "out">("in");

  function handlePutSnack({ name, description, date, time }: FormEditProps) {
    try {
      setIsLoading(true);
      StorageSnacks.createSnack(name, description, date, time, selectedInput);

      selectedInput === "in"
        ? router.push("/pages/FeedBackIn")
        : router.push("/pages/FeedBackOut");
    } catch (error) {
      showErrorToast(
        "Não foi possivel adicionar esta refeição, tente novamente."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <SnackContainer text="Nova Refeição">
      <KeyboardAvoidingView
